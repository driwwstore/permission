...
surat=$(curl -sS https://raw.githubusercontent.com/RMBL-VPN/permission/main/ip | grep -w $ipes | awk '{print $4}')
...
EOF
    ...
    fun_bot1() {
      [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
        wget -qO- https://raw.githubusercontent.com/RMBL-VPN/VIP/main/api >/etc/.maAsiss/.Shellbtsss
      }
